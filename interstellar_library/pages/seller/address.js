...
export default function Address({ data }) {
  ...
  const fetchData = async () =>{
    try {
      const response = await axios.get(`http://localhost:3000/seller/profile/address` );
  ...
  const handleSubmit_UserData = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put("localhost:3000/seller/profile/update_profile_info", Address_ID, Street, Building, City, Country, ZIP);
      if (response.data) {
    ...
    return (
      ...
      <_Layout>
        <div id="center_screen_Column" className="flex items-center justify-center h-screen">
                <form onSubmit={handleSubmit_UserData}>
            <div id="Columns_Holder" className="grid grid-cols-2 gap-4 w-full max-w-screen-lg mx-auto">
